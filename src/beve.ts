...
//@ts-nocheck
// Reading BEVE
export function readBeve(buffer: Uint8Array) {
    console.log("Beve Okunuyor")
    ...
    function reshape(array: any[], rows: number, cols: number) {
        const reshaped = [];
...
export function writeBeve(data: JSON) {
    const writer = new Writer();
    write_value(writer, data);
    return writer.buffer;
}

function write_value(writer: Writer, value: JSON) {
...
function writeCompressed(writer: Writer, N: number) {
    if (N < 64) {
